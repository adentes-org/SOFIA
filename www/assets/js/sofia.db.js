...
define(["jquery","pouchdb","moment","objectdiff","pouchdb-authentication","moment-locales"], function($,PouchDB,moment,objectDiff) {
  ...
  var db.fiches = {
    ...
    timeout : function() {
      ...
      */
      db.fiches.offlineAfterTimeout = window.setTimeout("console.log('Setting header color to offline');S.vue.router.app.$children[0].$data.options.backColor = S.config.header.backColorOffline;",5*1000);
    },
    watch : function(sync) {
    ...
    getByID : function(id) {
      var deferred = new $.Deferred();
      db.localDB.get(id).then(function (doc) {
        console.log(doc);
    ...
    getAllWithMine : function() {
        var deferred = new $.Deferred();
        db.fiches.getAll().then(function(result){
          result.my_fiches = db.fiches.filterMine(result.fiches);
    ...
    getAll : function() {
      var deferred = new $.Deferred();
      db.localDB.allDocs({include_docs: true,skip:0,limit:req_limit}).then(function (result) {
        // handle result
        var ret = {
